...
if uploaded_file is None:
    ...
    st.info("💡 **Awaiting chat upload...** Please select your WhatsApp chat log to begin.")
else:
    ...
    st.markdown('<div class="section-title">📁 Uploaded Chat Session</div>', unsafe_allow_html=True)
    ...
    if show_analysis:
        ...
        with col1:
            styles.render_metric_card("Total Messages", f"{num_messages:,}", selected_user, icon="✉️")
        with col2:
            styles.render_metric_card("Total Words", f"{num_words:,}", selected_user, icon="📝")
        with col3:
            styles.render_metric_card("Media Omitted", f"{num_media_messages:,}", selected_user, icon="🖼️")
        with col4:
            styles.render_metric_card("Links Shared", f"{num_urls:,}", selected_user, icon="🔗")

        # Finding busiest users in the group (Group Level)
        if selected_user == "All":
            st.markdown('<div class="section-title">👥 User Contribution & Activity</div>', unsafe_allow_html=True)
            ...
            with col_busy2:
                st.markdown("<div style='padding-top: 10px; font-weight:600; color:#a5b4fc; font-size:1.1rem; margin-bottom:8px;'>Percentage Contribution</div>", unsafe_allow_html=True)
                st.dataframe(new_df, use_container_width=True)

        # WordCloud and Common Words section using Tabs
        st.markdown('<div class="section-title">🔤 Vocabulary & Phrase Analysis</div>', unsafe_allow_html=True)
        ...
        if not all_words_df.empty:
            st.markdown("<div style='font-weight:600; color:#a5b4fc; font-size:1.1rem; margin-top:20px; margin-bottom:8px;'>Top 50 Most Common Phrases Data</div>", unsafe_allow_html=True)
            st.dataframe(all_words_df, use_container_width=True)

        # Emoji analysis section
        st.markdown('<div class="section-title">🎭 Sentiment & Emoji Analysis</div>', unsafe_allow_html=True)
        emoji_df = helper.emoji_helper(selected_user, df)
        
        if not emoji_df.empty:
            col_emo1, col_emo2 = st.columns([2, 3])
            with col_emo1:
                st.markdown("<div style='font-weight:600; color:#a5b4fc; font-size:1.1rem; margin-bottom:8px;'>Emoji Count Table</div>", unsafe_allow_html=True)
        ...
        st.markdown('<div class="section-title">📈 Interaction Timelines</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="section-title">⏱️ Activity & Engagement Patterns</div>', unsafe_allow_html=True)
        tab_days, tab_months, tab_heatmap = st.tabs(["📆 Weekly Activity Map", "🗓️ Monthly Activity Map", "🔥 Hourly Heatmap"])
        ...
        with tab_days:
            ...
            if busy_day is not None and len(busy_day) > 0:
                ...
                busy_day_fig.update_layout(coloraxis_showscale=False, xaxis_tickangle=-45)
        ...
        with tab_months:
            ...
            if busy_month is not None and len(busy_month) > 0:
                ...
                busy_month_fig.update_layout(coloraxis_showscale=False, xaxis_tickangle=-45)
